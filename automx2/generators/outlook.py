...
class OutlookGenerator(ConfigGenerator):
    ...
    def davserver_element(
        self, parent: Element, server: Davserver, login_name: str
    ) -> None:
    ...
    def mailserver_element(
        self, parent: Element, server: Server, login_name: str
    ) -> None:
        element = SubElement(parent, "Protocol")
        ...
        SubElement(element, "Port").text = str(server.port)
        SubElement(element, "LoginName").text = login_name
        SubElement(element, "SSL").text = self.on_off(
            socket_type_needs_ssl(server.socket_type)
        )
        if server.socket_type in ENCRYPTION_TYPE_MAP:
            ...
            SubElement(element, "Encryption").text = ENCRYPTION_TYPE_MAP[
                server.socket_type
            ]
    ...
    def client_config(self, local_part, domain_part: str, display_name: str) -> str:
        ...
        for server in self.servers_by_prio(domain.servers):
            ...
            name = expand_placeholders(
                self.pick_one(server.user_name, lookup_result.uid),
                local_part,
                domain_part,
            )
